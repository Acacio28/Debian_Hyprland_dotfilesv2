...
branch="main"
github_url="https://github.com/Acacio28/Debian_Hyprland_dotfilesv2/tree/$branch/config/hypr/"
  ...
  case "$response" in
    "action1")  
      if [ -d $KooL_Dots_DIR ]; then
        ...
        kitty -e bash -c "
          git clone --depth=1 https://github.com/Acacio28/Debian_Hyprland_dotfilesv2.git $KooL_Dots_DIR &&
          cd $KooL_Dots_DIR &&
          chmod +x copy.sh &&
          ./copy.sh &&
		  notify-send -u critical -i "$iDIR/ja.png" 'Update Completed:' 'Kindly log out and relogin to take effect'
